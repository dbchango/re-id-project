...
from utils.color_RGB import color_RGB
        ...
        while self.cap.isOpened():
            ...
            # (start) frame processing flow
                ...
                # (start) pre-processing flow
                    ...
                    mask_cp = crop_frame(x1, x2, y1, y2, mask_cp).astype('uint8') #silueta
                    cropped_frame = crop_frame(x1, x2, y1, y2, masked_image).astype('uint8') #color

                    silhouete_frame= color_RGB_var.grisColor(mask_cp)
                    color_frame = color_RGB_var.grisColor(cropped_frame)
                    # image filtering using LBP method
                    #bp_image = color_RGB_var.lbp(cropped_frame)   # normalization

                    # resizing images
                    silhouete_frame = cv2.resize(silhouete_frame, (40, 40))
                    color_frame = cv2.resize(color_frame, (40, 40))
                    print(silhouete_frame.shape)
                    # reshaping - because it will specify channels number
                    #silhouete_frame = silhouete_frame.reshape(40, 40, 3)
                    #color_frame = color_frame.reshape(40, 40, 3)
                    pre_processing_timer.end()
                # (end) pre-processing flow

                # (start) identification flow
                    identificator_timer.start()
                    predicted_name, accuracy = id_model.identify([[silhouete_frame], [color_frame]])
